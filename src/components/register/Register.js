...
const validationSchema = Yup.object({
  ...
  identityNo: Yup.number()
    ...
    .test("len", "Identity Number must be exactly 11 digits", (val) => val && val.toString().length === 11),
    birthDate: Yup.string().matches(/^\d{4}-\d{2}-\d{2}$/, 'Invalid date format').required('Date is required'),
  ...
  password: Yup.string().required("Required").min(3, 'Password must be at least 3 characters'),
});
...
function Register() {    
  ...
  const onSubmit = (values, { setSubmitting }) => {
    AuthenticationService.register(values.name, values.surname,values.identityNo,values.birthDate,values.phoneNumber,values.personType,values.monthlyIncome,values.password)
      ...
      <Form className="bg-white shadow-md rounded-lg px-12 pt-8 pb-8 mb-6 w-full max-w-lg">
        ...
        <div className="mb-4">
          ...
          <Field
            ...
            name="identityNo"
          />
          <ErrorMessage name="identityNo" className="text-red-500 text-sm" />
        </div>
        ...
        <div className="mb-4">
          ...
          <Field
            ...
            name="phoneNumber"
          />
          <ErrorMessage name="phoneNumber" className="text-red-500 text-sm" />
        </div>
